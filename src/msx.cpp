...
/* $Id: msx.cpp,v 1.3 2005/01/18 00:39:03 guest Exp $ */
...
u8 msx_read_8255(u32 address)
{
    ...
    case 3:
	printf("msx: 8255 read control (INVALID), returning 0xff.\n");
	return 0xff;
    }

    /* NOTREACHED */
    printf("msx: 8255 read function switch fault.\n");
    return 0xff;
}
...
void msx_write_8255(u32 address, u8 data)
{
    ...
    case 1:
	printf("msx: 8255 write port '%c' = 0x%02x.\n",
    ...
    case 3:
	if (data & 0x80) {
	    printf("msx: 8255 mode set 0x%02x.\n", data);
	} else {
#if 0
	    printf("msx: %s bit %d of port C.\n",
...
void msx_sync_mmu(void)
{
	...
	default:
	    printf("msx: switch failure in msx_sync_mmu();\n");
	    data = NULL;
	    break;
	}
...
memread8_t msx_read_function = (memread8_t)msx_read;
memwrite8_t msx_write_function = (memwrite8_t)msx_write;
...
{
event_scheduler* scheduler= new_event_scheduler(cal_event_delay_callback, msx_cpu);
...
void msx_init_cpu(void)
{
    ...
    msx_cpu->setiou(msx_cpu, (memread8_t)msx_io_read, (memwrite8_t)msx_io_write);
...
//    msx_memory = (unsigned char*)calloc(1, MSX_RAM_SIZE);
    msx_memory = new u8[MSX_RAM_SIZE];
...
/*
 * $Log: msx.cpp,v $
 * Revision 1.3  2005/01/18 00:39:03  guest
 * *** empty log message ***
 *
 * Revision 1.2  2005/01/15 20:32:01  guest
 * *** empty log message ***
 *
