...
/* $Id: pce_cd.cpp,v 1.1 2005/01/11 03:12:50 alainp Exp $ */
...
void pce_cd_command_0x03(void)
{
    ...
    printf("this shouldn't be happening, report it.\n");
...
void pce_cd_command_0xd9(void)
{
    ...
    if (pce_cd_command.args[9] != 0x40) {
	printf("pce_cd: play assumption broken, ignoring stop command.\n");
	pce_cd_1800 = 0xd8;
	return;
    }
...
/* 	printf("pce_cd: $1802 read, returning 0x%02x.\n", pce_cd_1802); */
	return pce_cd_1802;
    } else if (addr == 0x1803) {
	unsigned char retval;
	retval = pce_cd_1803;
	pce_cd_1803 = 0x00;
/*  	printf("pce_cd: $1803 read, returning 0x%02x.\n", retval); */
	return retval;
    } else if (addr == 0x1804) {
	printf("pce_cd: $1804 read, returning 0x%02x.\n", pce_cd_1804);
	return pce_cd_1804;
    } else if (addr == 0x1808) {
...
/*  	printf("pce_cd: memory fake.\n"); */
	...
	printf("pce_cd: unknown address read ($%04hx), returning 0x00.\n", addr);
	return 0x00;
    }
}
...
{
    if (addr == 0x1800) {
/*  	printf("pce_cd: $1800 = 0x%02x.\n", data); */
	if (data == 0x81) {
	    pce_cd_1800 = 0xd0;
	}
    } else if (addr == 0x1801) {
/*  	printf("pce_cd: $1801 = 0x%02x.\n", data); */
	if (data == 0x81) {
	    pce_cd_1800 = 0x40;
	}
	pce_cd_1801 = data;
    } else if (addr == 0x1802) {
/* 	printf("pce_cd: $1802 = 0x%02x.\n", data); */
...
/*  	printf("pce_cd: adpcm control 1 = 0x%02x.\n", data); */
	pce_cd_180b = data;
	if (data & 0x02) {
	    printf("pce_cd: AD_TRANS dma enable.\n");
...
/*  	printf("pce_cd: adpcm control 2 = 0x%02x.\n", data); */
	if (data & 0x01) {
	    /* FIXME: The full sequence is 0->3->2->0 on the low 2 bits */
/*  	    printf("pce_cd: adpcm wptr set 0x%04hx.\n", pce_cd_adpcm_data16); */
...
/*  	    printf("pce_cd: adpcm rptr set 0x%04hx.\n", pce_cd_adpcm_data16); */
	    pce_cd_adpcm_rptr = pce_cd_adpcm_data16;
	}
	if (data & 0x10) {
/*  	    printf("pce_cd: adpcm length set 0x%04hx.\n", pce_cd_adpcm_data16); */
	    pce_cd_adpcm_length = pce_cd_adpcm_data16;
	}
	if (data & 0x20) {
	    printf("pce_cd: adpcm play (don't you just wish).\n");
	    data &= ~0x20; /* a bit of a hack */
	}
	if (data & 0x80) {
	    printf("pce_cd: adpcm reset.\n");
	}
	pce_cd_180d = data;
    } else {
	printf("pce_cd: unknown address write ($%04hx = 0x%02x).\n", addr, data);
    }
}
...
void pce_cd_initialize(void)
{
    ...
    printf("pce_cd: initializing.\n");

    cd_init();

#if 1
    pce_mmap[0x68] = new u8[0x30000];
    for (i = 0x69; i < 0x80; i++) {
	pce_mmap[i] = pce_mmap[i-1] + 0x2000;
    }
#endif
    
    pce_mmap[0x80] = new u8[0x10000];
    for (i = 0x81; i < 0x88; i++) {
	pce_mmap[i] = pce_mmap[i-1] + 0x2000;
    }
...
/*
 * $Log: pce_cd.cpp,v $
 * Revision 1.1  2005/01/11 03:12:50  alainp
 * no message
 *
