...
void tms9918_writeport1(tms9918* vdp, unsigned char data)
{
	...
	if (data & 0x80) {
/* 	    printf("tms9918*: register write %d = %02x.\n", data & 7, vdp->addrsave); */
	    vdp->regs[data & 7] = vdp->addrsave;
	} else {
/* 	    printf("tms9918*: address write.\n"); */
...
void tms9918_render_line_mode_0(tms9918* vdp)
{
    ...
    cur_vbp = video_GetScanline(vdp->scanline);
	...
	color0 = vdp->pal[colortable[nametable[i] >> 3] & 15];
	color1 = vdp->pal[colortable[nametable[i] >> 3] >> 4];
	if (pattern & 0x80) {
	    *cur_vbp++ = color1;
	} else {
	    *cur_vbp++ = color0;
...
void tms9918_render_line_mode_1(tms9918* vdp)
{
    ...
    color0 = vdp->pal[vdp->regs[7] & 15];
    color1 = vdp->pal[vdp->regs[7] >> 4];
...
void tms9918_render_line_mode_2(tms9918* vdp)
{
    ...
    for (i = 0; i < 32; i++) {
	pattern = patterntable[nametable[i] << 3];
	color0 = vdp->pal[colortable[nametable[i] << 3] & 0x0f];
	color1 = vdp->pal[colortable[nametable[i] << 3] >> 4];
	if (pattern & 0x80) {
	    *cur_vbp++ = color1;
	} else {
...
void tms9918_render_line_mode_3(tms9918* vdp)
{
    ...
    for (i = 0; i < 32; i++) {
	pattern = patterntable[nametable[i] << 3];
	color0 = vdp->pal[pattern & 15];
	color1 = vdp->pal[pattern >> 4];
...
void tms9918_render_line(tms9918* vdp)
{
    ...
    if (vdp->regs[7] & 0x0f) {
	vdp->pal[0] = vdp->pal[vdp->regs[7] & 0x0f];
    } else {
	vdp->pal[0] = vdp->pal[1];
    }
    
    if (!(vdp->regs[1] & 0x40)) {
	memset(cur_vbp, vdp->pal[1], 256);
	return;
    }
...
void tms9918_init_palette(tms9918* vdp)
{
    ...
    for (i = 0; i < 16; i++) {
	vdp->pal[i] = vid_pre_xlat[i];
    }
}

tms9918* tms9918_create(void)
{
//tms9918* retval= (tms9918*)calloc(1, sizeof(tms9918));
tms9918* retval= new tms9918;

	if (retval) {
//		retval->memory = (unsigned char*)calloc(1, TMS_RAMSIZE);
		retval->memory = new u8[TMS_RAMSIZE];
		if (retval->memory) {
			retval->scanline = 0;
