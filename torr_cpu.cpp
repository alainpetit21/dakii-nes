...
/*     static int trace = 0; */
	...
	if (trace) {
	    printf("torrigion: executing opword 0x%04hx from 0x%06lx.\n", opword, context->pc - 2);
	}
#endif
	if (opword == 0x60fe) {
	    printf("torrigion: stopping execution at infinite loop at 0x%06x.\n", context->pc - 2);
	    system_flags |= F_UNIMPL;
	}
/* 	if (opword == 0x3036) { */
/* 	    printf("torrigion: 0x3036!.\n"); */
/* 	} */
/* 	printf("torrigion: executing opword 0x%04hx from 0x%06lx.\n", opword, context->pc - 2); */
/* 	printf("torrigion: executing opword 0x%04hx\n", opword); */
...
/*     printf("torr_68k: SR = 0x%04hx.\n", context->flags); */
	...
	if ((new_sr ^ context->flags) & 0x2000) {
	    /* FIXME: switch stacks */
	    printf("BUG: stack switch on IRQ not implemented\n");
	}
